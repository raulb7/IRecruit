<?php
namespace Eti\JobeetBundle\Tests\Repository;
...
class JobRepositoryTest extends WebTestCase
{
    ...
    {
        $query = $this->em->createQuery('SELECT c FROM EtiJobeetBundle:Category c');
        $categories = $query->getResult();
 
        foreach($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) FROM EtiJobeetBundle:Job j WHERE j.category = :category AND j.expiresAt > :date');
            $query->setParameter('category', $category->getId());
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $jobs_db = $query->getSingleScalarResult();
 
            $jobs_rep = $this->em->getRepository('EtiJobeetBundle:Job')->countActiveJobs($category->getId());
    ...
    public function testGetActiveJobs()
    {
        ...
        foreach ($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) from EtiJobeetBundle:Job j WHERE j.expiresAt > :date AND j.category = :category');
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $query->setParameter('category', $category->getId());
            $jobs_db = $query->getSingleScalarResult();
 
            $jobs_rep = $this->em->getRepository('EtiJobeetBundle:Job')->getActiveJobs($category->getId(), null, null);
    ...
    {
        $query = $this->em->createQuery('SELECT j FROM EtiJobeetBundle:Job j WHERE j.expiresAt > :date');
        $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
        $job_db = $query->getSingleResult();
 
        $job_rep = $this->em->getRepository('EtiJobeetBundle:Job')->getActiveJob($job_db->getId());
        ...
        $job_rep = $this->em->getRepository('EtiJobeetBundle:Job')->getActiveJob($job_expired->getId());
